...
void OLED_ShowFloat(uint8_t Line, uint8_t Column,float Parameter,u8 i)    //把可变参数Parameter以小数类型显示
{                                                                 //i代表小数点后保留i位
	  char buf[20];
	  switch(i){
			case 1:sprintf(buf,"%.1f",Parameter); break; 
	    case 2:sprintf(buf,"%.2f",Parameter); break;   //保留两位小数,并以逐个单字符存入数组buf中,包括小数点   Parameter=buf
	    case 3:sprintf(buf,"%.3f",Parameter); break;
			case 4:sprintf(buf,"%.4f",Parameter); break;
			case 5:sprintf(buf,"%.5f",Parameter); break;
			case 6:sprintf(buf,"%.6f",Parameter); break;
			case 7:sprintf(buf,"%.7f",Parameter); break;
			case 8:sprintf(buf,"%.8f",Parameter); break;
			default :break;
		}
